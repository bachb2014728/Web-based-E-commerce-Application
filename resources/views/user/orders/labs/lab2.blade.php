    ...
    <div class="card-body">
      ...
      <div class="border bg-light p-2 mb-2 rounded-3">
        ...
        <div class="d-flex flex-row-reverse bd-highlight">
          <a href="{{route('order.edit', ['root' => '0', 'id' => $items->id])}}" class="btn btn-sm label"><i data-feather="edit"></i></a>
          <form action="{{ route('profile.removeOrder', $items->id) }}" method="POST" type="button" onsubmit="return confirm('Bạn chắc chắn hủy đơn hàng này chứ?')">
            @csrf
            @method('post')
            <button class="btn m-0 btn-sm label"><i data-feather="trash-2"></i></button>
          </form>
...
<div class="tab-foot">
  @if ($users_0->count() > 0)
  <p class="text-end m-2"><strong>Tổng số hóa đơn : <span class="badge bg-secondary">{{$users_0->count()}}</span></strong> | Tổng tiền các hóa đơn : <strong id="price">{{$prices}}</strong></p> 
  @endif
</div>